...
```bash
# Use the Meta Rust plugin for Cargo workspace operations
meta rust build
...
### Build Everything
```bash
meta rust build --release
```

### Run Tests (Parallel for Speed)
```bash
meta --parallel git status
meta --parallel rust test
```
...
# Exclude slow repos
meta --exclude large-monorepo exec -- sh -lc 'cargo update'
```
...
meta rust test             # runs in auth-fix's repos

# Override with --primary to use primary checkout paths
meta --primary rust test
```
...
- **Dependency order**: Use `--sequential` when a command must not run concurrently; order cross-repo dependencies explicitly in the invoking workflow.
- **Avoid unnecessary parallel**: Don't use `--parallel` for operations with cross-repo dependencies — sequential execution is safer.
