...
# Target by tag
meta --tag backend rust test

# Exclude repos
meta --exclude legacy exec -- npm update

# Sequential execution; invoke provider groups before their dependents
meta --sequential rust build
...
3. **Build in order**: invoke providers before dependents explicitly; use `meta --sequential rust build` for each ordered group.
...
meta --strict git worktree create feature-test --from-ref v2.0.0 --all
meta --strict git worktree prune
meta --strict rust build
```
...
When using `meta git worktree exec --ephemeral`, the automatic cleanup after command execution intentionally uses best-effort mode (`strict=false`), even if you specified `--strict`. This ensures:
...
If you need to ensure cleanup completed fully, follow up with `meta git worktree list` to verify.
...
meta git worktree create feature-test --from-ref v2.0.0 --all --strict
```
...
- `meta --sequential rust build` prevents concurrent builds; invoke dependency groups explicitly.
