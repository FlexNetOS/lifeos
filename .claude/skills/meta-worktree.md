...
meta git worktree create auth-fix --repo backend --repo frontend

# Create with all repos in the workspace
meta git worktree create full-task --all

# Specify branch name (default: task name)
meta git worktree create my-task --repo api --branch feature/my-feature

# Per-repo branch override
meta git worktree create review --repo api:main --repo worker:develop

# Start from a specific tag/SHA
meta git worktree create incident-42 --from-ref v2.3.1 --repo api

# Start from a GitHub PR's head branch
meta git worktree create review --from-pr org/api#42 --repo api
```
...
meta git worktree create ci-check --all --ephemeral

# Set TTL for automatic expiration
meta git worktree create ci-check --all --ttl 1h

# Store custom metadata
meta git worktree create ci-check --all --meta agent=review-bot --meta run_id=abc123

# Atomic create + run + destroy
meta git worktree exec --ephemeral lint-check --all -- make lint
```
...
meta git worktree list
meta git worktree list --json

# Show detailed status of a worktree
meta git worktree status auth-fix

# Show diff vs base branch
meta git worktree diff auth-fix --base main

# Add a repo to existing worktree
meta git worktree add auth-fix --repo another-service
```
...
meta git worktree exec auth-fix -- sh -lc 'cargo test'

# Filter repos
meta git worktree exec auth-fix --include backend -- make build
meta git worktree exec auth-fix --exclude legacy -- npm test

# Run in parallel
meta git worktree exec auth-fix --parallel -- sh -lc 'cargo build'
```
...
```bash
meta git worktree create auth-fix --repo . --repo backend --repo frontend
```
...
meta git worktree remove auth-fix

# Force destroy (even with uncommitted changes)
meta git worktree remove auth-fix --force

# Prune expired/orphaned worktrees
meta git worktree prune
meta git worktree prune --dry-run  # preview without removing
```
...
steps:
  - name: Review PR
    run: |
      meta git worktree exec --ephemeral review-${{ github.run_id }} \
...
- **Ephemeral for CI**: `meta git worktree exec --ephemeral` creates, runs, and destroys in one atomic operation
