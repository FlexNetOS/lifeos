...
| `https://github.com/FlexNetOS/rtk-tokenkill` | `main` | `e446998cafe1c20eb23a371f65e6f5c3ed6f16fa` | rtk `0.43.0` plus `rtk_nu` byte-first adapter; envelope `flexnetos.rtk_nu.envelope.v1` |
...
- rtk and rtk_nu: `rtk` is the inspected Rust output/token/cost proxy. `rtk_nu` is the separately packaged raw-byte tee and JSONL/Nuon adapter pinned in `rtk-tokenkill` at `e446998c…`; it is not a native Nu value producer or plugin child.
