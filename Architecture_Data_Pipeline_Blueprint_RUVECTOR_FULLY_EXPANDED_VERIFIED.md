...
The repository-backed terminal entrypoint is `yzx enter [zellij-args...]`. The activated LifeOS stack pins `portable-pty` 0.9.0 in the Tauri Rust backend, `@xterm/xterm` 6.1.0-beta.292 with `@xterm/addon-fit` 0.12.0-beta.292, `@xterm/addon-image` 0.10.0-beta.292, and `@xterm/addon-webgl` 0.20.0-beta.291 in the Svelte pane, and ordered Tauri IPC channels for PTY output. The PTY controller runs Nushell `-l -c ^yzx enter --session <database-derived-name>`, owns the master handle, forwards input and resize commands, streams ordered byte-array frames to xterm.js, and records start, attach/create, resize, input, output, exit, signal, and error frames. xterm.js is only the renderer; the actual Nushell/Zellij/Helix/Yazi processes run behind the PTY. The database-derived name binds tenant/session/branch identity so a restarted Glass reconnects to the same Engine Room. Tauri global events are reserved for small control notices; PTY bytes use ordered channels because Tauri documents events as JSON and not suitable for high-throughput/low-latency streams. The current LifeOS checkout is Svelte/Tauri, and its build, PTY argv, owner projection, reconnect, resize, byte-forwarding, and native tests provide the current activation evidence; the remaining owner-protocol and release-gate requirements below remain explicit.

#### 3.1.1 Glass VT capability envelope

`@xterm/xterm` above names **xterm.js**, the renderer embedded in the Svelte pane. It is not XTerm the X11 application and it is not a host terminal emulator. Kitty, Ghostty, WezTerm, Alacritty, and Yazelix Nova's bundled Mars (Rio-derived) occupy the *native* front door one layer out, where they host `yzx enter` directly. The two front doors are parallel and neither substitutes for the other: a GPU host emulator cannot mount inside a webview pane, and xterm.js cannot replace the native host. Host terminal choice therefore remains open at the native front door; Mars is the Yazelix-native reference host because the `yazelix-zellij` Kitty passthrough bridge is tuned against it.

Because the two front doors render the same Engine Room, the Glass renderer's capability envelope is normative and must be declared, not inherited. The PTY controller exports a `TERM` that describes **the embedded renderer**, and strips the host's `TERM_PROGRAM`/`TERM_PROGRAM_VERSION`. Inheriting the host `TERM` is a silent downgrade: a Ghostty host would cause Yazi to negotiate against the host's capabilities instead of the embedded renderer's Kgp compatibility envelope, and would invite Helix and Zellij to negotiate a keyboard protocol with no responder.

The activated envelope is:

| Capability | Value | Basis |
|---|---|---|
| `TERM` exported to the Engine Room | `xterm-256color` | truthful terminfo baseline for the embedded renderer |
| Host terminal identity | the **complete** marker set removed — `TERM_PROGRAM`, `TERM_PROGRAM_VERSION`, `GHOSTTY_*`, `KITTY_WINDOW_ID`, `KITTY_PID`, `KITTY_INSTALLATION_DIR`, `KONSOLE_VERSION`, `ITERM_SESSION_ID`, `WEZTERM_*`, `WARP_HONOR_PS`, `VTE_VERSION`, `WT_SESSION`, `ALACRITTY_*`, `LC_TERMINAL*` | `CommandBuilder` seeds from `std::env::vars_os()`; Yazi's brand detection reads this whole set, not `TERM_PROGRAM` alone |
| Pixel-geometry queries (`CSI 16 t`, `CSI 14 t`) | answered | xterm.js defaults `windowOptions` to `{}` and otherwise ignores them; Yazi requires cell size before it will draw |
| Kitty keyboard protocol | supported, negotiated at runtime via `CSI ? u` | `@xterm/xterm` `kittyKeyboard` |
| Kitty graphics protocol | supported (direct placement) | `@xterm/addon-image` `kittySupport` |
| Kitty Unicode placeholders (`U+10EEEE`) | implemented by the Glass renderer compatibility addon | Kgp virtual placements are adapted before xterm.js parsing; raw PTY capture remains unchanged |
| SIXEL · iTerm2 IIP | supported | `@xterm/addon-image` |
| Image protocol selected for Yazi | `Kgp` | the renderer-side compatibility addon adapts virtual placements for the pinned image addon |
| PTY pixel geometry | reported every resize | image protocols size from it |

**Stripping the host identity is what activates the envelope, and it must be complete.** Yazi resolves its image adapter from the host-identity environment variables *first*, and negotiates against the terminal's own query responses only when none of them are present. A single surviving marker therefore defeats the whole envelope: verified against Yazi 26.5.6, leaving `GHOSTTY_RESOURCES_DIR` in place makes Yazi report `Brand.from_env: Some(Ghostty)` and select an adapter for the host emulator rather than the renderer drawing the pixels — on this host `Adapter.matches: Chafa`, which is not installed, so **no image renders at all**. With the full set stripped, Yazi reports `Brand.from_env: None`, derives `sixel: true` from the renderer's own DA1 reply (`@xterm/addon-image` answers `CSI ?62;4;9;22c`), and renders for real. Evidence: `evidence/glass/engine-room-image-render-receipt.json`, reproducible via `scripts/probe-yazi-image-adapter.py`.

**Which image protocol is operative in the Engine Room is Yazi's decision, not the renderer's.** Yazi disables Kgp whenever it detects a terminal multiplexer, and it reads `ZELLIJ_SESSION_NAME`. The Engine Room front door is `yzx enter` → Zellij, so Yazi inside Glass will not select Kgp however capable the renderer is: with every Kitty signal aligned (`KITTY_WINDOW_ID`, `TERM=xterm-kitty`, an XTVERSION reply of `kitty(0.42.0)`, and the cell-size query answered) `Adapter.matches` remained `Chafa` while `ZELLIJ_*` was present, and became `Kgp` the moment those variables were hidden. Sixel is therefore the operative Engine Room path by Yazi's own multiplexer policy — not a downgrade chosen here. The Kgp compatibility adapter stays required and correct for any non-multiplexed Kgp producer rendering into Glass.

**Placement positioning is verified, not assumed.** Kgp positions an image by its `U+10EEEE` placeholder-cell region while a direct placement paints at the cursor, so the renderer-side conversion is only sound if the cursor already sits at that region's origin. Measured against a live 712 KB Yazi Kgp stream (181 APC sequences, 676 placeholder cells): Yazi parks the cursor at the region origin before emitting the placement — `2;64` against origin `2;64`, and `3;64` against origin `3;64` on redraw — so the converted placement lands exactly on the placeholder region. Replaying that stream through the shipped adapter changes it by exactly eight bytes, the two occurrences of `U=1,`, and leaves every cursor sequence, placeholder cell, and chunked `m=1` continuation identical. Yazi already sets `C=1` itself, which the adapter preserves rather than duplicating. Evidence: `evidence/glass/kgp-placement-position-receipt.json`.

`YAZI_IMAGE_PROTOCOL=Kgp` is exported for image-protocol-aware consumers, but it is **not** the authority for adapter selection: Yazi 26.5.6 does not read it (its only `YAZI_*` inputs are `YAZI_CONFIG_HOME`, `YAZI_FILE_ONE`, `YAZI_ID`, `YAZI_LEVEL`, `YAZI_LOG`, `YAZI_PID`, `YAZI_ZOXIDE_OPTS`). Kitty graphics remain implemented end to end — the image addon's `kittySupport` plus the `U+10EEEE` placeholder adapter — so any producer emitting Kgp still renders; Yazi 26.5.6 simply sets `kgp: true` only from a recognised emulator brand, which this envelope deliberately does not claim. The envelope is a single constant (`GLASS_VT_PROFILE`) published to the renderer through a Tauri command so backend, renderer, and verification read one source. Terminal bytes remain unmodified end to end: the renderer adapts only its display stream, performs no end-of-line rewriting, and forwards 8-bit input unchanged. The Kgp compatibility addon consumes the `U+10EEEE` placeholder cells after xterm.js parsing while the image addon paints the direct placement; raw PTY capture remains byte-complete.
...
The owner serializes mutations, serves consistent snapshots from its open handle, and emits ordered commit notifications containing `local_seq`, transaction/table/key scope, projection generation, mmap generation, and checksum. This single-owner service is lifecycle-independent of the Glass so terminal/background ingestion continues while LifeOS is closed. A reconnect first verifies the mapped generation, then requests missing events or a fresh snapshot through the authenticated Unix-domain protocol. There is no conventional local HTTP state server and no claim that redb itself supplies cross-process notifications. The activated `flexnetos-redb-owner` implementation publishes the mmap projection, enforces path/token/protocol checks, detects stale readers, falls back on checksum failure, and replays a commit after a publish interruption; LifeOS-native live tests exercise these paths. Release gates still require the pinned closure, full cross-process deployment, and measured SLO evidence.
...
The pinned `rtk-tokenkill` source provides the `rtk` binary and compact human-readable filters plus the separately packaged `rtk_nu` binary at revision `eee0dfbd3cf3dc82f5604c77ccc4f93c4a5f0c45`. It does not universally emit JSON, JSONL, Nuon, or native Nu tables through `rtk` itself. No `components/rtk_nu` implementation exists in the pinned `meta` tree; the adapter is owned and pinned in `rtk-tokenkill`. `rtk_nu` remains a separately versioned FlexNetOS adapter whose exact envelope, package, and witness coordinates are part of activation.
...
The `nu_plugin` workspace provides CodeDB core, cargo/static analysis, context, build capture, MCP, redb storage, PostgreSQL storage, fixtures, and the Nushell child plugin. It is the byte-complete ingress and query foundation. Nushell and the child exchange plugin protocol frames through `MsgPackSerializer`; CodeDB then maps validated values and linked raw objects to its storage schemas. The current command inventory includes `codedb ingest-envelope` with typed record/list/table input, exact-byte validation and canonical raw-object receipts; targeted live tests cover the command, malformed-input rejection, deduplication, and redb round trip. MessagePack transport is distinct from redb storage encoding. Although `codedb-store-pg` implements direct PostgreSQL persistence, authoritative production writes are denied to plugin/agent roles and are invoked only by envctl; other direct-store uses are explicitly granted bootstrap, parity, repair, migration, or read-only operations.
...
envctl is the only authoritative PostgreSQL/RuVector ingress commit bridge and is also the materializer, synchronization boundary, configuration-table operator, security boundary, projection manager, and activation interface. The permanent operational flow is `database rows ↔ validated envctl tables ↔ Nushell tables ↔ controlled files`, with original source files retained as evidence. The activated commit worker drains ordered redb/CodedB outbox records, validates policy and idempotency, invokes a database-authorized embedding executor when a target schema requires vectors, invokes the CodeDB PostgreSQL mapping with envctl-only credentials, records the commit identity, and returns the ordered acknowledgement/projection. Live integration tests cover deterministic ownership, ordered idempotent restart, durable acknowledgement, and denial of non-envctl writes. ruvllm/Candle is the default approved local embedding path; ONNX/ort is optional only in a separately approved closure. Capability enforcement, rollback, and full activation remain governed by the release gates below.
...
| `https://github.com/FlexNetOS/nu_plugin` | `master` | `c49af6e5a9301296d9ff0133c04acd987363155f` | CodeDB `0.1.0`, Nu `0.113.1`, ingest-envelope, redb-owner and PostgreSQL stores |
| `https://github.com/FlexNetOS/rtk-tokenkill` | `main` | `eee0dfbd3cf3dc82f5604c77ccc4f93c4a5f0c45` | rtk `0.43.0` plus `rtk_nu` byte-first adapter; envelope `flexnetos.rtk_nu.envelope.v1` |
...
- LifeOS current checkout: Svelte Glass in `src/`, Tauri/PTY controller and owner projection bridge in `src-tauri`, plus `lifeos-core` and `lifeos-daemon`; Svelte 5, Vite, Tauri 2, Bun 1.3.14, `portable-pty` 0.9.0, and pinned xterm addons. The compatibility Vue/Pinia siblings remain only for legacy preview paths; the mounted application entry is Svelte.
...
- rtk and rtk_nu: `rtk` is the inspected Rust output/token/cost proxy. `rtk_nu` is the separately packaged raw-byte tee and JSONL/Nuon adapter pinned in `rtk-tokenkill` at `eee0dfbd…`; it is not a native Nu value producer or plugin child.
...
2. Provision the adjacent meta graph (`loop_lib`, `meta_plugin_protocol`, `meta-ruvector`), lane, Yazelix/Nix/Nushell, Rust toolchains, Bun, PostgreSQL libraries, cargo-pgrx, approved ruvllm/Candle executors, optional separately gated ONNX Runtime, CUDA/NVIDIA, and musl linkers. Pin and witness `rtk_nu` at the verified `rtk-tokenkill` revision and implement, package, pin, and witness the `flexnetos-redb-owner` service, including its versioned mmap projection format, atomic multi-slot publisher, permissions, event protocol, stale-reader recovery, and corruption fallback.
...
13. Activate the LifeOS Tauri/Svelte Glass with the Rust PTY controller, the real `yzx enter`/Zellij Engine Room, and the sidebar bound to the redb-owner mmap projection plus matching commit events. Prove atomic generation publication, mapped-byte/checksum equality, stale-reader and owner-restart recovery, input/resize/output fidelity, no local HTTP live-state server, no direct concurrent redb database-file open, and the canonical CodeDB/envctl/PostgreSQL return route; the current checkout supplies the implementation and native/build evidence, while these acceptance proofs remain release gates.
...
| redb | cberner/embedded Rust ecosystem; FlexNetOS integration | `https://github.com/cberner/redb`; FlexNetOS repos | `redb`; `codedb-store-redb`; `flexnetos-redb-owner` and mmap projection publisher | application 4.1; RuVector internal pin 2.1; projection format implemented and pinned by owner crate | Pure-Rust ACID file backend, MVCC, zero-copy API, transactional buffer, scratchpad, cache, dedupe, application WAL, geometry/SIMD, owner-published mmap live read model | Active local boundary below the canonical Swarm Primary Runtime | One supervised owner holds the only writable database handle; ingress, cache, geometry, retry, WAL, atomic read-only mmap generations, fallback snapshots and ordered UDS events | Stores envctl events/checkpoints/rollback, commit receipts and sync cursors | Backend-neutral store and parity source; mutation clients use owner IPC; LifeOS maps verified read-model generations | Caches AgentDB/RVF/model/route live state without durable authority | Frames, vectors, cache keys, retries, projections, operations | Atomic mmap generations, consistent snapshots, ordered events, distance results, reconciliation batches | Every byte, table record, mmap generation, cache entry, vector, transition, retry, WAL payload and event | redb 4.1 database file is not mmap-backed; owner-published read-only mmap projection; authenticated UDS; application SIMD | 6 | Clients ↔ owner/database and mapped read model ↔ envctl-only commit; monotonic local-seq/mmap-generation/LSN reconciliation | LifeOS-native live tests: single-owner denial; token/protocol rejection; atomic/checksum projection; ordered replay; crash republish; corruption fallback; latency samples; PostgreSQL reconciliation |
| envctl | FlexNetOS | `https://github.com/FlexNetOS/envctl` | agent-env, engine, CLI, GUI, secrets crates and daemons; durable drain/commit worker | 0.1.0; revision `e9112b22…`; worker implemented and live-tested | Bidirectional bridge, detection, tables, projection, embedding orchestration, security, materialization, activation, rollback | Only authoritative PostgreSQL/RuVector ingress committer; consumes canonical rows and returns every byte | Drains owner sequences; stores evidence/checkpoints/commit receipts/rollback cursors | Direct role: exclusive committer, table operator, security boundary, projection and activation | Invokes CodeDB PG mapping with envctl-only credentials after validation/idempotency | Orchestrates approved database-authorized embedding executor; optional ONNX/ort separately gated | Ordered local records, files, DB rows, discovery, policy, leases, grants | Committed rows/LSN/witness, validated tables, files, worktrees, activation evidence | Original/generated bytes, vectors, table generations, diffs, checkpoints, secret lifecycle | `ruvector,ruvector-pg,ruvector-cuda`; redb 4.1; Rust 1.89 | 6 | redb owner → envctl validate/embed/map/commit → PostgreSQL → owner acknowledgement | Live integration: deterministic owner projection; durable acknowledgement; ordered idempotent restart; non-envctl write denial |
...
| nu_plugin / CodeDB | FlexNetOS | `https://github.com/FlexNetOS/nu_plugin` | CodeDB crates/stores/MCP/`nu_plugin_codedb`; `codedb ingest-envelope` | 0.1.0; revision `c49af6e5…`; Nu 0.113.1; command and owner path implemented | Byte-complete trees/blobs, typed Nu child protocol via MsgPackSerializer, Cargo/static/build capture, redb/PG parity, MCP | Typed ingress/query foundation; production roles have no authoritative PG-write credential | Owner-client backend replaces competing direct file opens | Provides exports/contracts; only envctl may invoke production PG mapping | Typed record/list/table envelope validation, raw links, canonical receipt, malformed-input rejection and deduplication | Imports model/AgentDB/RVF and returns RuVector context | Raw-object IDs, Nu values, files/trees, builds, queries, contexts | Durable object IDs, semantic rows, query projections, proof manifests | File/binary/symlink/mode/BOM bytes, AST, build logs, plugin frames and provenance | `pg-integration`; isolated Nu config/DB roles; typed-input command | 7 | Nu typed value → required ingest child → owner → envctl-only PG mapping → PostgreSQL | Live targeted tests: JSON/JSONL/Nuon/native-input contracts; raw-link; PG denial; store parity/tree reconstruction |
...
| LifeOS and Codex | FlexNetOS/OpenAI | `https://github.com/FlexNetOS/lifeos` | Tauri/Svelte, `portable-pty`, xterm.js/WebGL, Tauri channels, mmap projection reader, core/daemon/Codex | LifeOS 0.1.0; current checkout; dependencies pinned | Glass sidebar, embedded real Yazelix/Zellij Engine Room, mapped live projection, controlled actions, agent workflow | Starts requests and displays database-controlled results; owns no truth | Maps verified owner-published read-only generations and receives ordered events; never opens the writable database file | Receives generated app/MCP/session configuration | Durable writes traverse Engine/Nu/plugin or Tauri CodeDB envelopes then envctl | Presents agents, `.rvf` pressure, tasks, sessions, Git/RuVector branches, models/forecasts | User input, PTY byte arrays, mapped generations/events, Codex context | Ordered PTY/channel frames, requests, tool calls, UI effects, responses | Interactions, terminal frames, mapped/render generations, model exchanges, acks/errors | Svelte/Tauri 2, portable-pty 0.9.0, `@xterm/xterm` 6.1.0-beta.292, fit 0.12.0-beta.292, image 0.10.0-beta.292, WebGL 0.20.0-beta.291 | 13 | xterm.js ↔ Tauri channel/PTY ↔ Nushell `-l -c ^yzx enter --session <database-derived-name>`; mmap generation + owner event → sidebar | Live build/native tests; exact argv; PTY/resize/reconnect/bytes; mmap checksum/stale-reader; no HTTP/direct database open; latency |
...
| lifeos component: src-tauri | FlexNetOS | https://github.com/FlexNetOS/lifeos | src-tauri | 0a7b77afc9f40a459a54b1965c544101059870f3; PTY/channel and mmap-reader implementation live | `portable-pty` controller, redb-owner client, and verified read-only mmap projection reader | Database-owned request/result UI projection | Maps owner-published generation and consumes matching events; never a concurrent database-file opener | Builds/configures/secures/materializes exact component closure | Captures source/request/PTY/input/output/mmap/render/artifact/provenance | Codex, agents, RVF pressure, ruvllm results, forecasts | Task/branch/session/policy/lease plus PTY input/resize bytes and projection generation | Ordered Tauri-channel PTY frames, mapped generations, owner events, result/error/witness | Every source/config/input/output/cache/log/mmap/render/artifact byte and record | Tauri 2; portable-pty 0.9.0; Svelte/xterm lock pins and projection-format pin | 13 | xterm ↔ ordered channel ↔ PTY ↔ exact `yzx enter options ...`; mmap generation + owner UDS event → sidebar | Live build/native tests; exact argv; reconnect/resize/bytes; atomic-map/checksum/stale-reader; no HTTP/direct database open |
...
| lifeos component: lifeos-vue | FlexNetOS | https://github.com/FlexNetOS/lifeos | legacy `lifeos-vue` compatibility siblings; mounted target is Svelte Glass | 0a7b77afc9f40a459a54b1965c544101059870f3; compatibility path retained | Vue/Vite/Pinia siblings remain only for legacy preview compatibility; the mounted application is the Svelte sidebar plus terminal pane | Database-owned request/result UI projection | Production target reads Tauri-verified mmap generations and subscribes to matching owner events | Builds/configures/secures/materializes exact component closure | Captures UI input, mapped/render generation, output, error and provenance | Presents agents, `.rvf` pressure, tasks, sessions, Git/RuVector branches | User input, PTY state, typed mmap projections/events | Rendered Glass state, actions, acknowledgements/errors | Every source/config/input/mmap/render/event/error byte and record | Svelte 5/Tauri 2/Bun 1.3.14; Vue/Pinia compatibility siblings | 13 | mmap generation + owner event → Tauri → Svelte store; Tauri PTY → terminal pane | Build/native/a11y evidence; mmap/render/event ordering; full Glass/Engine acceptance |
...
| network-control component: netengine | FlexNetOS | https://github.com/FlexNetOS/network-control | netengine | cad70349968d2a6f501fb304a77d5cd6dac340b5 | Network inventory/planning/SNMP/SSH/lane/obscura/iroh/apply/rollback/GUI | Executes database-authorized network plans | Live buffer/cache/read/ledger state; every byte reconciles | Builds/configures/secures/materializes exact component closure | Captures complete source/request/input/output/log/state/artifact/provenance | Network/device state for swarms, ruvllm, ATAS | Database task/branch/policy/lease plus exact source/runtime bytes | Complete result/state/log/artifact/witness stream | Every source/config/input/output/cache/log/state/artifact byte and record | Rust 1.89; snmp,relay,hostnet | 12 | PostgreSQL ↔ envctl/CodeDB/redb ↔ component ↔ PostgreSQL return | Pinned revision; locked package tests; interruption/replay and byte round trip |
| network-control component: netctl | FlexNetOS | https://github.com/FlexNetOS/network-control | netctl | cad70349968d2a6f501fb304a77d5cd6dac340b5 | Network inventory/planning/SNMP/SSH/lane/obscura/iroh/apply/rollback/GUI | Executes database-authorized network plans | Live buffer/cache/read/ledger state; every byte reconciles | Builds/configures/secures/materializes exact component closure | Captures complete source/request/input/output/log/state/artifact/provenance | Network/device state for swarms, ruvllm, ATAS | Database task/branch/policy/lease plus exact source/runtime bytes | Complete result/state/log/artifact/error/witness stream | Every source/config/input/output/cache/log/state/artifact byte and record | Rust 1.89; snmp,relay,hostnet | 12 | PostgreSQL ↔ envctl/CodeDB/redb ↔ component ↔ PostgreSQL return | Pinned revision; locked package tests; interruption/replay and byte round trip |
| network-control component: netctl-gui | FlexNetOS | https://github.com/FlexNetOS/network-control | netctl-gui | cad70349968d2a6f501fb304a77d5cd6dac340b5 | Network inventory/planning/SNMP/SSH/lane/obscura/iroh/apply/rollback/GUI | Executes database-authorized network plans | Live buffer/cache/read/ledger state; every byte reconciles | Builds/configures/secures/materializes exact component closure | Captures complete source/request/input/output/log/state/artifact/provenance | Network/device state for swarms, ruvllm, ATAS | Database task/branch/policy/lease plus exact source/runtime bytes | Complete result/state/log/artifact/error/witness stream | Every source/config/input/output/cache/log/state/artifact byte and record | Rust 1.89; snmp,relay,hostnet | 12 | PostgreSQL ↔ envctl/CodeDB/redb ↔ component ↔ PostgreSQL return | Pinned revision; locked package tests; interruption/replay and byte round trip |
...
| rtk_nu | FlexNetOS | `https://github.com/FlexNetOS/rtk-tokenkill` | `rtk_nu` | `eee0dfbd…`; `flexnetos.rtk_nu.envelope.v1`; package and binary pinned | Lossless pre-transform byte tee plus versioned JSONL/JSON/Nuon envelope; never native Nu/plugin child | Carries legacy-output evidence into database-controlled ingestion | Reaches owner only after Nushell parsing and plugin validation | Builds/configures/secures/materializes adapter | Nu/plugin links raw objects, typed values, effects and provenance | Supplies cost/error/context signals without replacing raw evidence | Raw stdout/stderr/binary frames and execution identity | JSONL/JSON/Nuon text, lossless base64 frames, provisional frame IDs/digests, parse/filter/exit/timing metadata | Every original frame/order/digest plus derived representation and error | Rust/Nix package, schema and witness pin present | 10 | command → `rtk_nu` tee + `rtk` compact branch → envelope → Nu parse → plugin → owner | Live binary/stderr and envelope suite; invalid UTF-8/binary/interleave/parse-failure/native-bypass tests |
...
| lane/obscura/iroh | FlexNetOS/network | https://github.com/FlexNetOS/network-control | network fabric | cad70349968d2a6f501fb304a77d5cd6dac340b5 | On-host/off-host network/peer relay | Database-controlled execution and canonical state/results | Live state/operation buffer; every record reconciles | Builds/configures/secures/materializes database-selected projection | Captures source/request/input/output/effect/log/artifact/provenance/witness | Connects to ruvllm/AgentDB/RVF/SONA/FastGRNN/Ruflo/RuvLTRA/ATAS by role | Database task/branch/identity/policy/lease/exact bytes/projection | Complete result/state/effect/error/artifact/acknowledgement/witness | All source/config/input/output/cache/log/model/artifact/effect bytes/records | Exact repo/package/Nix-lock coordinate | 12 | PostgreSQL ↔ envctl/CodeDB/redb ↔ component ↔ PostgreSQL return | Pinned revision/closure; component test; byte/effect/provenance/witness query |
...
**Addendum 2026-07-23:** rows R17–R19 append a reconciliation re-review against current workspace HEADs. Two 2026-07-19 release blockers — `codedb ingest-envelope` and `rtk_nu` — are now implemented in-repo; R01 (Vue→Svelte) is re-affirmed open. Implementation acknowledgement does not narrow any release gate (invariant 16 still governs pin/schema/witness).

**Activation correction 2026-07-28:** `rtk_nu` is now pinned and exercised from `rtk-tokenkill` `main` revision `eee0dfbd3cf3dc82f5604c77ccc4f93c4a5f0c45`. The binary emits `flexnetos.rtk_nu.envelope.v1`, preserves binary and invalid-UTF-8 frames as base64 with length/digest/offset/order metadata, and passes the repository envelope suite plus a live binary/stderr run. The historical R06/R07/R14/R18/R19 entries retain their original evidence date; the current pin and runtime evidence supersede only their “unimplemented/unpinned” status.
...
LifeOS R01 currency is reconciled at current revision `10865e60`: `src/main.ts` mounts `App.svelte`, the production command reports zero Svelte diagnostics and a successful Vite build, the isolated live Vite mount serves the `#app` entrypoint, and the Tauri library tests pass both terminal argv and redb-event validation cases. The Vue/Pinia siblings remain compatibility-only; the mounted Glass is Svelte.

**Latency activation correction 2026-07-28:** ARCHBP-130 and ARCHBP-120 now consume the current `yazelix/envelope/yzx-envelope.nu` engine or the released bundle shim rather than deleted planning-spine artifacts. The committed `evidence/benchmarks/envelope_latency_benchmark.json` records 9 self-timed runs with `-1.11%` runtime overhead and 63 ms median setup; the released-bundle record measures `-0.21%` overhead and 32 ms setup. Both focused Vitest suites pass 3/3, and the benchmark workload executes successfully inside the released bundle with the host store hidden.

**Packaging activation correction 2026-07-28:** ARCHBP-134 and ARCHBP-021 now consume the committed `evidence/packaging/portable_release_packaging_decision.json` and `evidence/packaging/portable_release_root_coverage.json` receipts instead of deleted planning-spine documents. The live release gate re-verifies three static-pie eligible binaries, classifies six non-musl dependencies, runs the moved-prefix launcher with the host store hidden, and runs a static workload with an empty `/nix` surface; the focused packaging and portable-release suites pass 7/7.

**R01 activation correction 2026-07-28:** `scripts/verify-svelte-entrypoint.mjs` now proves the production closure from `index.html` to `src/main.ts` to `src/App.svelte`, requires the Svelte Vite plugin, rejects Vue mount/import markers from emitted JavaScript, and records the compatibility-only Vue/Pinia dependency role in `evidence/glass/svelte-entrypoint-receipt.json`. `bun run verify:svelte-entrypoint` passes against the built assets. This supersedes the historical R01 “open” finding for the mounted target while preserving the original review row and its compatibility-path caveat.

**Capability correction 2026-07-28:** R03 examined terminal *transport* (ordered Tauri channels vs JSON events) but never examined the renderer's *capability envelope*. That omission left `TERM` inherited from the host process, so a Ghostty host advertised `xterm-ghostty`/`TERM_PROGRAM=ghostty` to every process in the Engine Room while the embedded renderer could not draw Kitty graphics or answer Kitty keyboard queries. §3.1.1 now declares the envelope normatively, `GLASS_VT_PROFILE` implements it as one constant, and `bun run verify:terminal-capability` gates it. Row R20 records the finding. This supersedes no part of R03's transport conclusion, which stands.

**Current-state reconciliation 2026-07-29:** R01 is closed for the mounted production target: `index.html → src/main.ts → src/App.svelte` is the live Svelte closure, and `bun run verify:svelte-entrypoint` rejects a Vue mount/import path in emitted assets. Vue/Pinia remain compatibility-only preview dependencies. R11 is closed for the implemented commit boundary: Tauri supervises a loopback-only native `@ruvector/ruvllm` executor when envctl reconciliation is enabled, `envctl-commit-worker` enriches staged jobs before witnessing, and a live staged sequence was committed through the `lifeos_envctl` role into PostgreSQL with a 128-dimensional vector and matching acknowledgement. The original R01 and R11 rows remain historical snapshots; this reconciliation does not narrow invariant 16 or any release gate.

**Adapter and launch reconciliation 2026-07-29:** The active `rtk_nu` source coordinate is `rtk-tokenkill@eee0dfbd3cf3dc82f5604c77ccc4f93c4a5f0c45`, matching `integrations/rtk_nu.toml`, the package source, and the live envelope/CodeDB receipts; the older `e446998c…` coordinate remains only in historical review evidence. The active Glass launch gate is `bun run verify:glass-launch-live`; it requires a fresh mounted Svelte readiness record and a fresh `lifeos.engine-room.ready` timestamp from the same Tauri launch, preventing stale Zellij readiness from being accepted as current activation. A fresh launch, Engine Room probe, and authenticated owner projection all pass.

**CodeDB pin reconciliation 2026-07-29:** The immutable LifeOS integration target is the clean `nu_plugin` checkout at `5ec4242c656d019ea9dd583c1c78f1d5d48b4e7f`, a formatting-only descendant of the owner-socket implementation pin `5cc8c97b1b05a47949af455c951570fbee9701e4`. LifeOS does not mutate that checkout. The current pin passes the live `rtk_nu → CodeDB → redb` envelope path and the ARCHBP-042 receipt/test; the prior pin remains historical evidence only.

**Full live-gate reconciliation 2026-07-29:** PostgreSQL/RuVector authority (17.10, 87 migrations), byte-complete arbitrary-binary reconstruction with SHA-256/SHAKE256 parity and witness verification, envctl-owned OpenPencil durable Apply with idempotency/path/digest guards and maintained projection, tenant-scoped byte-linked lexical retrieval, dimension-specific dense retrieval with populated HNSW/ruivfflat projections, durable hybrid lexical+dense fusion with reconstructable component/ranking ledgers, and the single approved RuVector library identity all pass from the current checkout, alongside RuVector learning/compatibility repairs, PITR (18/18), dirty-shutdown recovery, COW branch/frontdoor promotion and rollback, envctl-only CodeDB commit, native embedding, secret lifecycle, redb-owner latency, Engine Room, fresh Glass launch/UI readiness, NBVERIFY-004 closure, native RuVector node authority, and the complete frontend/a11y/design/diagram release check. The active PostgreSQL 17 profile now carries the rebuilt full-feature 0.3.1 artifact; the live feature matrix applies the canonical compatibility repairs, source-identifier sparse aliases, and extension ownership reconciliation, and exercises 384/384 extension objects with 336/336 passing cases, zero uncovered objects, and zero absent source-anchor names. Seven names remain explicitly accounted as other live object forms or compatibility surfaces.

**Full-feature dependency and SQL correction 2026-07-29:** The pinned RuVector `pg17,all-features-v3` graph now uses `fastembed` with Rustls Hub transport, image-model support, and dynamic ONNX loading; the graph contains no `openssl-sys` or `native-tls`. Full-feature SQL defaults are emitted as valid PostgreSQL literals (quoted text, JSONB cast, and numeric curvature), the anchored sparse Rust identifiers are exposed as SQL aliases, and the approved runtime artifact exposes 369 generated functions. This is landed in `meta-ruvector` `c68d05bbd` (the target-gated RVF WASM panic-handler descendant of `875ccb2fd`) and the envctl vendored dependent `6a6159ad`. Yazelix’s sole Nix profile now builds and activates that source at `lifeos_foundation_yzx`; `bun run verify:ruvector-full-feature-closure` records exact coordinates, active profile artifact hashes, and live extension ownership.

**Network-control authority closure 2026-07-29:** The existing append-only `lifeos_coord.network_plan` and `network_effect` envelopes now expose lease-, task-, branch-, grant-, and tenant-bound `submit_network_plan`, `start_network_plan`, and `record_network_effect` procedures in migration 0098. The `lifeos-network-control-executor` claims only a database-started plan, invokes the exact `netctl` argv without a shell, records raw stdout/stderr bytes, and executes the declared rollback argv when the primary operation fails. The live database applied migration 0098 successfully; the active `network-control` checkout is `cad70349968d2a6f501fb304a77d5cd6dac340b5`, its workspace check is green, and real `netctl --json status` plus a non-mutating `link set lo up` dry run returned structured results. An unauthorized live submission was rejected by the active lease boundary.

**Weave/runner coordination authority closure 2026-07-29:** The existing append-only `lifeos_coord.weave_job`, `weave_attempt`, `runner_job`, and `runner_receipt` envelopes now expose lease-, task-, branch-, grant-, and tenant-bound submission, start, fencing, and receipt procedures in migration 0099. The `lifeos-coordination-executor` claims only a database-started Weave or runner request, invokes its exact argv without a shell, preserves stdout/stderr bytes, and records the result through the canonical procedures. The live database reports migrations 98 and 99 applied and all six coordination procedures present. The active Weave checkout is `9805ab9b7473ca662dd24f81a564f0e29ebb7e49`; the active rusty-idd checkout is `fd95efdd3d461874ceab28a4ad82c435e162b751`. Native Weave and rusty-idd local stores remain consumer projections; PostgreSQL/RuVector is the dispatch, lease, attempt, and receipt authority.
...
| R17 | R09/R14; §§3.4, 4.5, 14; invariant 16 | 2026-07-23 re-review: `codedb ingest-envelope` was recorded unbuilt on 2026-07-19. | nu_plugin `@931d48f`: `crates/codedb/src/ingest.rs` (schema `codedb.ingest-envelope.v0`, tagged ARCHBP-001), `crates/nu_plugin_codedb/src/main.rs:3308` command registration, `crates/codedb/src/main.rs:170` typed-receipt handler, `crates/codedb_store_redb/src/lib.rs` content-addressed persistence. | The required typed-ingest surface is now implemented against this blueprint. | R09/R14 recorded it as an unbuilt, activation-blocked release blocker; that snapshot is now stale. | Reconciled forward (additive): implementation acknowledged; R09/R14 preserved as the historical pre-correction record. No gate narrowed. | Exact revision pin + SHAKE256 witness + release-gate execution per invariant 16 before "implemented" is accepted at release. |
| R18 | R06/R07/R14; §§3.4, 4.4; invariant 16 | 2026-07-23 re-review: `rtk_nu` "neither implementation exists" on 2026-07-19. | rtk-tokenkill: `src/rtk_nu_main.rs` (pre-transform byte capture, envelope `flexnetos.rtk_nu.envelope.v1`, clap `name = "rtk_nu"`); Cargo crates `rtk_nu` and `rtk_nu_test_fixture`. | The required adapter is now implemented with the exact envelope contract R06/R07 specified. | R07 recorded it as a new required adapter, activation-blocked pending pin/witness; that snapshot is now stale. | Reconciled forward (additive): implementation acknowledged; R06/R07 preserved. No gate narrowed. | Exact package/revision + schema closure + witness pin per invariant 16 before release. |
| R19 | Review-ledger preamble; R01, R08, R11, R12 | 2026-07-23 currency re-review of the 2026-07-19 pins. | HEAD deltas: nu_plugin `63d68743→931d48f`, lifeos `3d741436→4707497`, envctl now on branch `codex/profile-xdg-owner`; lifeos `package.json` = `"vue": "^3.5.34"`. | Pins are 4 days stale; R01 (Vue→Svelte) re-verified STILL OPEN; R08/R11/R12 unverified against current revisions. | Ledger snapshot lagged repository advancement. | Recorded the currency delta (additive); R01 re-affirmed OPEN; R08/R11/R12 re-review scheduled as required work. | Re-review each named row against its current pinned revision; R01 closes only when the Svelte target replaces the Vue entrypoint per its build-closure gate. |
| R20 | §3.1, §3.1.1; R03 | The Glass renderer's capabilities are adequately specified by pinning `@xterm/xterm`, and "xterm" denotes a terminal choice open to Kitty/Ghostty/Mars. | LifeOS `src-tauri/src/lib.rs` (PTY `TERM` inheritance), `src/components/EngineRoomTerminal.svelte`; [`@xterm/addon-image`](https://www.npmjs.com/package/@xterm/addon-image) `0.10.0-beta.292` typings and bundle; [Yazi image preview](https://yazi-rs.github.io/docs/image-preview/); [Zellij kitty passthrough](https://github.com/zellij-org/zellij/issues/4500) | "xterm" is xterm.js, an embedded renderer, not a host emulator — Kitty/Ghostty/Mars are not substitutable at that layer. Versions were pinned but no capability envelope was declared, so the PTY inherited the host `TERM` (`xterm-ghostty` here) and `TERM_PROGRAM`. Yazi then selected Kitty Unicode placeholders, which the renderer does not implement. Pixel geometry was reported as `0`, `convertEol` rewrote line endings, and `onBinary` input was dropped. | R03 proved transport but not renderer capability; the resulting mismatch is a silent downgrade barred by invariant 16 and the byte-exact contract. | Added §3.1.1 as the normative envelope; `GLASS_VT_PROFILE` is the single constant, host terminal identity is stripped, `YAZI_IMAGE_PROTOCOL=KgpOld` is set explicitly, pixel geometry is reported, `convertEol` is off, and `onBinary` is forwarded. Kitty Unicode placeholders recorded as a declared release-gated gap. | `bun run verify:terminal-capability` (pins ↔ profile ↔ renderer, host-TERM rejection, fallback-drift, byte-fidelity) writing `evidence/glass/terminal-capability-receipt.json`; Rust `terminal_tests` host-terminal/`TERM_PROGRAM`/protocol cases; `tests/EngineRoomTerminal.capability.spec.js`. |
| R21 | R01, R11; §§3.1, 3.2, 4.7 | Current mounted Glass and envctl embedding state match the target architecture. | `evidence/glass/svelte-entrypoint-receipt.json`, `evidence/envctl-codedb/embedding-live-receipt.json`, commits `db3c61e2` and `11c4a776` | The historical ledger snapshots predated the active Svelte closure and native embedding boundary. | Current-state evidence was not yet reconciled into the ledger. | Closed R01 and R11 additively while retaining their historical rows and all invariant-16 release requirements. | Fresh Svelte entrypoint gate; native RuvLLM health/embed; real staged envctl commit with 128-dimensional vector and matching PostgreSQL acknowledgement. |
| R22 | §§3.1–3.3; R02/R03 | A Glass launch could be accepted using an old Engine Room readiness record even when the mounted UI itself was fresh. | `scripts/verify-glass-launch-live.mjs`, `src/App.svelte`, `src/components/EngineRoomTerminal.svelte`, fresh `evidence/glass/live-launch-receipt.json` | The launch verifier checked Engine Room schema/state but not readiness freshness relative to the current Tauri launch. | Stale Zellij readiness could masquerade as current Engine Room activation. | Require `lifeos.engine-room.ready.observedAt >= launch.startedAt`; expose the gate as `bun run verify:glass-launch-live`. | Fresh Tauri launch, fresh Engine Room probe, authenticated owner projection, and clean SIGTERM shutdown. |
| R23 | §3.1.1; R20 | The Kgp Unicode-placeholder finding is resolved at the correct layer. | `src/lib/kitty-unicode-placeholders.js`, `src/components/EngineRoomTerminal.svelte`, `src-tauri/src/lib.rs`, `tests/kitty-unicode-placeholders.spec.js` | The pinned image addon performs direct Kitty placement but does not natively consume Kgp `U+10EEEE` virtual placeholder cells. | Selecting `KgpOld` avoided the missing path but did not implement Yazi's default Kgp contract. | Added a renderer-only compatibility stream/addon: Kgp placement commands are adapted before xterm.js parsing, placeholder cells are cleared after parsing, and the raw PTY capture remains untouched. The backend now selects `YAZI_IMAGE_PROTOCOL=Kgp`. | Protocol split-frame tests, capability verifier, Svelte capability tests, Rust terminal profile tests, and live Glass launch gate. |
| R24 | R08/R12; §§3.3–3.4, 4.5, 4.7; invariant 16 | The current typed-ingress path preserves raw bytes through adapter, Nu parsing, CodeDB, redb, and envctl commit. | Active `nu_plugin` pin `5ec4242c…`, `verify:rtk-nu`, `verify:codedb-rtk-nu`, and `verify:envctl-codedb-live` receipts; protected checkout remains clean | R08/R12 evidence was tied to superseded 2026-07-19 plugin sources and did not cover the current owner-socket path end to end. | Current revision, envelope schema, raw-object receipt, frame metadata, and envctl-only durable commit needed one current-state closure. | Reconciled additively: current adapter/Nu/CodeDB/redb byte path and envctl PostgreSQL return path are live; R08/R12 historical rows remain unchanged and invariant 16 remains fully enforced. | `verify:rtk-nu` 7/7 envelope tests; `verify:codedb-rtk-nu` raw-object receipt; `verify:envctl-codedb-live` four committer tests including non-envctl write denial. |
| R25 | §3.1.1; R20/R23; invariant 16 | The Glass VT envelope was fully activated once `GLASS_VT_PROFILE`, the Kgp compatibility adapter, and `verify:terminal-capability` were in place. | `src-tauri/src/lib.rs` (`apply_glass_vt_profile`), `src/components/EngineRoomTerminal.svelte`, live `yazi` 26.5.6 under a PTY reproducing the envelope, `evidence/glass/engine-room-image-render-receipt.json` | The envelope was declared but only partially enforced. `apply_glass_vt_profile` stripped `TERM_PROGRAM`/`TERM_PROGRAM_VERSION` only, so `GHOSTTY_RESOURCES_DIR` survived from the host session; Yazi resolves its adapter from the host-identity set *before* negotiating with the terminal, reported `Brand.from_env: Some(Ghostty)`, and selected `Adapter.matches: Chafa` — absent on this host, so the Engine Room rendered no image at all. Separately, xterm.js defaults `windowOptions` to `{}`, leaving Yazi's `CSI 16 t` cell-size query unanswered (`Terminal response timeout`, no scale reference), and `YAZI_IMAGE_PROTOCOL` is not a variable Yazi 26.5.6 reads, so the declared `image_protocol` had no effect on selection. | Partial identity stripping is inheritance by omission and is the silent downgrade §3.1.1 exists to prevent: the rendered result depended on which emulator happened to host the session rather than on the declared envelope. | `HOST_TERMINAL_IDENTITY_VARS` now strips the complete marker set; the renderer answers the pixel-geometry queries via read-only `windowOptions`; §3.1.1 records that stripping is what activates negotiation and that `YAZI_IMAGE_PROTOCOL` is not the authority. Kitty graphics support is retained end to end — no protocol capability was removed. | `bun run verify:terminal-capability` (asserts the full strip list is declared *and* applied, plus the window-op reports); Rust `terminal_tests` `glass_vt_profile_strips_every_host_emulator_marker` and `host_terminal_identity_covers_yazi_brand_detection`; `tests/EngineRoomTerminal.capability.spec.js` window-option cases; `scripts/probe-yazi-image-adapter.py` before/after — `Chafa`/no image → `Sixel`/38 KB rendered. |
| R26 | §3.1.1; R23/R25 | The Kgp compatibility adapter's virtual→direct conversion positions images correctly, and Kgp is the Engine Room's image path. | Live yazi 26.5.6 Kgp capture replayed through `src/lib/kitty-unicode-placeholders.js`; `evidence/glass/kgp-placement-position-receipt.json` | Positioning had never been observed — W1's launch blocker and then Yazi's adapter choice prevented it. Measurement now confirms the conversion is positionally correct: the cursor is at the placeholder region origin at placement time in every redraw, and the adapter alters the stream by exactly the two `U=1,` occurrences while leaving cursor sequences, placeholder cells and chunked continuations byte-identical. Separately, Yazi disables Kgp under a multiplexer and reads `ZELLIJ_SESSION_NAME`, so inside `yzx enter` → Zellij it selects Sixel regardless of renderer capability. | An unverified positioning claim would have been a silent visual defect; and asserting Kgp as the Engine Room path misstates which protocol actually carries Engine Room images. | No adapter change was required — the conversion was already correct. §3.1.1 now records the multiplexer policy and the positioning measurement, and three regression tests derived from the real capture pin the behaviour. | `tests/kitty-unicode-placeholders.spec.js` (real yazi placement control, chunked continuations, cursor-sequence preservation); `evidence/glass/kgp-placement-position-receipt.json`. |
...
> **Note on counts:** the phrase "ten operational invariants" in the anchor conformance ledger (A15) and in item 19 below denotes the **anchor's** invariant set, rendered across the D01–D24 atlas. The numbered list in this section is this document's **nineteen** acceptance invariants, which expand and operationalize the anchor's ten. The two counts are complementary, not contradictory.
