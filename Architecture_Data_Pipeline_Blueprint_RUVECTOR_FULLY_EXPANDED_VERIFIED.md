...
| Kitty Unicode placeholders (`U+10EEEE`) | **not implemented** — declared gap | absent from the pinned addon |
| SIXEL · iTerm2 IIP | supported | `@xterm/addon-image` |
| Image protocol selected for Yazi | `KgpOld` | the Kitty variant the renderer implements |
| PTY pixel geometry | reported every resize | image protocols size from it |

`YAZI_IMAGE_PROTOCOL=KgpOld` is set explicitly rather than left to Yazi's terminal detection, because Yazi's `Kgp` default is the Unicode-placeholder variant. The envelope is a single constant (`GLASS_VT_PROFILE`) published to the renderer through a Tauri command so backend, renderer, and verification read one source. Terminal bytes remain unmodified end to end: the renderer performs no end-of-line rewriting and forwards 8-bit input unchanged. The Kitty Unicode-placeholder gap is a declared, release-gated delta under invariant 16 and must not be described in silent-downgrade language.
