...
LIFEOS_DATABASE_URL="postgresql://lifeos_app@database.example/lifeos" \
LIFEOS_RUNTIME_TENANT_ID="<envctl-issued-tenant>" \
LIFEOS_RUNTIME_IDENTITY_ID="<envctl-issued-identity>" \
LIFEOS_RUNTIME_GRANT_ID="<envctl-issued-grant>" \
LIFEOS_RUNTIME_BINDING_JSON="<envctl-issued-binding-json>" \
LIFEOS_ENGINE_SESSION_NAME="<envctl-derived-zellij-session-name>" \
bun run tauri:dev
```

The Tauri runtime pool binds every PostgreSQL connection through
`lifeos_security.bootstrap_envctl_context` before it is handed to application
code. The four `LIFEOS_RUNTIME_*` values are envctl-issued session material;
they are not generated by LifeOS and are required after database cutover.
`LIFEOS_ENGINE_SESSION_NAME` is the envctl-derived, tenant/session/branch-bound
Zellij name used by the Glass PTY to reattach to the same Engine Room.
