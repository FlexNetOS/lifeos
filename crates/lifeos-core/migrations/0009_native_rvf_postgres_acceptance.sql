-- Native RVF/PostgreSQL closure for INV-011.
--
-- Keep the versioned database receipt and native receipt as independent
-- append-only facts. The public capability opens only when both receipts are
-- content-address valid, the native evidence is bound to the selected database
-- receipt, and every live RuVector C-library catalog binding has a SHA-256
-- digest in that evidence.
