-- Restore the INV-011 semantic self-check after migration 0017 replaced the
-- UUID/blob-v2-compatible body installed by 0016 with its stale BIGINT body.
-- This is deliberately forward-only: 0005/0006 remain checksum-pinned.
...
BEGIN
  WITH required(name) AS (
    VALUES
      ('lifeos_runtime.branch'), ('lifeos_runtime.branch_overlay'),
      ('lifeos_agent.branch_witness'), ('lifeos_runtime.merge_gate'),
      ('lifeos_runtime.merge_conflict'), ('lifeos_runtime.merge_conflict_resolution'),
      ('lifeos_runtime.merge_conflict_application'), ('lifeos_runtime.promotion'),
      ('lifeos_runtime.branch_pointer'), ('lifeos_runtime.cow_request'),
      ('lifeos_runtime.cow_request_result'), ('lifeos_runtime.canonical_projection'),
      ('lifeos_runtime.cow_acceptance_receipt'), ('lifeos_rvf.container'),
      ('lifeos_rvf.cow_map'), ('lifeos_rvf.membership'),
      ('lifeos_rvf.member_vector_identity'), ('lifeos_rvf.branch_roundtrip_receipt')
  )
  SELECT count(*) INTO table_count FROM required WHERE to_regclass(name) IS NOT NULL;
  ...
  SELECT count(*) INTO function_count FROM required WHERE to_regprocedure(name) IS NOT NULL;
  ...
  WHERE relation.relforcerowsecurity
    ...
    AND relation.relname IN (
      ...
      'merge_conflict', 'merge_conflict_resolution', 'merge_conflict_application',
      'promotion', 'branch_pointer', 'cow_request', 'cow_request_result',
      'canonical_projection', 'container', 'cow_map', 'membership',
      'member_vector_identity', 'branch_roundtrip_receipt'
    );

  SELECT count(*) INTO legacy_witness_count
  FROM lifeos_agent.branch_witness witness WHERE witness.preimage_version = 0;

  -- COW v2 was installed before S16 renamed its append-only BIGINT blob
  -- relation to object_pre_s16. Resolve the witnessed representation rather
  -- than assuming the newer UUID object catalog owns those references.
  IF to_regclass('lifeos_blob.object_pre_s16') IS NOT NULL THEN
    EXECUTE $sql$
      SELECT count(*)
      FROM lifeos_agent.branch_witness witness
      JOIN lifeos_blob.object_pre_s16 preimage ON preimage.id = witness.preimage_object_id
      WHERE witness.preimage_version = 1
        AND witness.entry_shake256 <> extensions.ruvector_shake256_256(preimage.raw_bytes)
    $sql$ INTO invalid_witness_count;
  ELSE
    SELECT count(*) INTO invalid_witness_count
    FROM lifeos_agent.branch_witness witness
    JOIN lifeos_blob.object preimage ON preimage.object_id = witness.preimage_object_id
    WHERE witness.preimage_version = 1
  ...
  WITH ordered AS (
    SELECT witness.branch_id, witness.sequence, witness.previous_shake256,
      lag(witness.entry_shake256) OVER (
        PARTITION BY witness.branch_id ORDER BY witness.sequence
      ) AS expected_previous
    FROM lifeos_agent.branch_witness witness WHERE witness.preimage_version = 1
  )
  SELECT count(*) INTO invalid_chain_count FROM ordered
  WHERE previous_shake256 <> coalesce(expected_previous, decode(repeat('00', 32), 'hex'));

  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'lifeos_envctl') THEN
    SELECT count(*) INTO old_envctl_execute_count FROM (VALUES
    ...
    ) legacy(name) WHERE has_function_privilege('lifeos_envctl', name, 'EXECUTE');
  END IF;

  RETURN jsonb_build_object(
    'forced_rls_count', forced_rls_count, 'function_count', function_count,
    ...
    'ready', table_count = required_table_count AND function_count = required_function_count
      AND forced_rls_count = 17 AND legacy_witness_count = 0
      AND invalid_witness_count = 0 AND invalid_chain_count = 0
      AND old_envctl_execute_count = 0,
    'required_function_count', required_function_count,
    'required_table_count', required_table_count, 'table_count', table_count
  );
END
$function$;
