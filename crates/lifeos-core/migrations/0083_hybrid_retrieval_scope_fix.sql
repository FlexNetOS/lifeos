-- LifeOS migration 0083 — qualify the fused rank in the window ordering.
-- The function's OUT parameter named fused_rank otherwise collides with the
-- ranked CTE column under PL/pgSQL name resolution.
...
BEGIN
  ...
  WITH lexical AS (
    ...
    SELECT candidates.*, row_number() OVER (
      ORDER BY candidates.fused_rank DESC, candidates.source_object_id, candidates.byte_start
    )::integer AS position
    FROM candidates
  ), persisted AS (
    INSERT INTO lifeos_semantic.retrieval_result (
      query_id, rank_position, source_object_id, byte_start, byte_end,
      lexical_rank, dense_distance, dense_rank, fused_rank, discarded, component_payload
    )
    SELECT query_uuid, position, source_object_id, byte_start, byte_end,
           lexical_rank, dense_distance, dense_rank, fused_rank, position > p_limit,
           jsonb_build_object('query_text', p_query_text, 'fusion_config', p_fusion_config,
