...
import { 
  ...
  ArrowRight, Activity, FolderGit2, X, Lock, RefreshCw, MessageSquare, Wrench
} from 'lucide-react';
...
export default function App() {
  ...
  // NEW STATE FOR GLOBAL CONFIG
  const [isGeneratingGlobalConfigs, setIsGeneratingGlobalConfigs] = useState(false);
...
Respond ONLY with a JSON object strictly matching this schema. Do not include markdown code blocks wrapping the JSON:
{
  ...
  const generateTaskConfig = async (task) => {
    ...
    try {
      ...
      const payload = {
        contents: [{
          role: "user",
          parts: [{ text: `You are a configuration generation tool. Analyze the following runbook task. If this task involves creating or modifying a configuration file (like YAML, JSON, TOML, .env, GitKB configs, etc.), generate the optimal literal contents for that file based on the context provided.
          
Task ID: ${task.id}
Task Title: ${task.title}
Objective: ${task.objective}
Commands: ${(task.commands || []).join('\n')}
Files Touched: ${(task.files_touched || []).join(', ')}

Respond ONLY with a JSON object. If a config is needed, provide "filename" and "content". If no config is needed for this specific task, provide a "message" explaining why.` }]
        }],
        generationConfig: {
          responseMimeType: "application/json",
          responseSchema: {
      ...
      if (result.candidates && result.candidates[0].content?.parts[0]?.text) {
        ...
        setGeneratedConfigs(prev => ({ ...prev, [task.id]: data }));
      }
    } catch(err) {
      console.error("Config generation failed:", err);
      setGeneratedConfigs(prev => ({ ...prev, [task.id]: { message: "Error connecting to AI. Could not build config." } }));
    } finally {
...
${JSON.stringify(runbook.tasks.filter(t => t.status !== 'completed').map(t => ({id: t.id, title: t.title, commands: t.commands})), null, 2)}`;
  ...
  // NEW FUNCTION FOR GLOBAL CONFIGS
  const generateGlobalConfigs = async () => {
    if (!runbook) return;
    setIsGeneratingGlobalConfigs(true);
    ...
    try {
      ...
      const payload = {
        contents: [{
          role: "user",
          parts: [{ text: `You are a GitKB configuration architect. Analyze the following compiled runbook. Generate the necessary global project configuration files required to execute this runbook safely. 
          Specifically, look at the task types, commands, and agent notes. 
          Generate a JSON object containing an array of files. Each file should have a "path" (e.g., ".kb/config.toml", "AGENTS.md", ".env.example") and "content".
          
          Project Name: ${config.projectName}
          Target Agent: ${config.agent}
          
          Runbook Tasks Summary:
          ${JSON.stringify(runbook.tasks.map(t => ({id: t.id, title: t.title, commands: t.commands, objective: t.objective})), null, 2)}
      ...
      if (result.candidates && result.candidates[0].content?.parts[0]?.text) {
        ...
        // Export the generated configs as a JSON file for the user to review
        const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(data, null, 2));
        const downloadAnchorNode = document.createElement('a');
        downloadAnchorNode.setAttribute("href", dataStr);
        downloadAnchorNode.setAttribute("download", "gitkb-global-configs.json");
        document.body.appendChild(downloadAnchorNode);
        downloadAnchorNode.click();
        downloadAnchorNode.remove();
      }
    } catch(err) {
      console.error("Global config generation failed:", err);
      alert("Failed to generate global configurations.");
    } finally {
      setIsGeneratingGlobalConfigs(false);
    }
  };
                    ...
                    <div className="space-y-3 pt-4 border-t border-slate-800/80">
                      <div className="flex items-center justify-between">
                        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center">
                           <Wrench className="w-4 h-4 mr-2" /> AI Config Builder
                        </h3>
                        {!generatedConfigs[activeTask.id] && (
                          <button 
                            onClick={() => generateTaskConfig(activeTask)}
                            disabled={isGeneratingConfig}
                            className="text-[10px] bg-blue-900/30 text-blue-400 hover:bg-blue-900/50 border border-blue-900/50 px-2.5 py-1.5 rounded transition-all disabled:opacity-50 flex items-center font-bold"
                          >
                            {isGeneratingConfig ? <RefreshCw className="w-3 h-3 mr-1.5 animate-spin" /> : <Wrench className="w-3 h-3 mr-1.5" />}
                            {isGeneratingConfig ? 'Building...' : 'Auto-Build Config'}
                          </button>
                        )}
                      </div>
                      
                      {generatedConfigs[activeTask.id] && (
                        <div className="bg-slate-950 border border-slate-800 rounded-md p-4 shadow-inner">
                          {generatedConfigs[activeTask.id].message ? (
                             <p className="text-xs text-slate-500 italic flex items-center">
                               <AlertTriangle className="w-3.5 h-3.5 mr-2 text-slate-600" />
                               {generatedConfigs[activeTask.id].message}
                             </p>
                          ) : (
                             <div>
                                <div className="flex items-center justify-between mb-3 border-b border-slate-800/50 pb-2">
                                   <div className="flex items-center">
                                      ...
                                      <span className="text-xs font-bold font-mono text-emerald-400">{generatedConfigs[activeTask.id].filename}</span>
                                   </div>
                                   <button onClick={() => {
                                       navigator.clipboard.writeText(generatedConfigs[activeTask.id].content);
                                       alert(`${generatedConfigs[activeTask.id].filename} copied to clipboard!`);
                                   }} className="text-slate-500 hover:text-slate-300 transition-colors bg-slate-900 p-1.5 rounded border border-slate-800" title="Copy File Content">
                                       <Copy className="w-3.5 h-3.5" />
                                   </button>
                                </div>
                                <pre className="text-[12px] text-slate-300 font-mono bg-[#050505] p-4 rounded border border-slate-800/80 overflow-x-auto whitespace-pre-wrap leading-relaxed">
                                   {generatedConfigs[activeTask.id].content}
                                </pre>
                             </div>
                          )}
                        </div>
        ...
        {runbook && (
          <footer className="h-16 border-t border-slate-800 bg-slate-950 flex items-center justify-between px-6 shrink-0 absolute bottom-0 left-0 right-0 z-30 shadow-[0_-10px_20px_-5px_rgba(0,0,0,0.3)]">
            <div className="flex items-center space-x-4">
              ...
              {/* NEW EXPORT BUTTON */}
              <button 
                onClick={generateGlobalConfigs} 
                disabled={isGeneratingGlobalConfigs}
                className="flex items-center space-x-1.5 px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-emerald-400 rounded text-xs font-semibold transition-colors border border-emerald-900/50 shadow-sm disabled:opacity-50"
              >
                {isGeneratingGlobalConfigs ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Wrench className="w-3.5 h-3.5" />}
                <span>Build Global Configs</span>
              </button>
            </div>
