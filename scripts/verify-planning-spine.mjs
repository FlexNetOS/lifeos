...
const requiredDocs = [
  ...
  "1.0_VISION/ARCHITECTURE_BLUEPRINT_TASK_COVERAGE.md",
  "1.0_VISION/Architecture_Anchors/README.md",
  "1.0_VISION/Architecture_Anchors/Architecture_Data_Pipeline_Blueprint_RUVECTOR_FULLY_EXPANDED_VERIFIED.md",
  "1.0_VISION/Architecture_Anchors/Architecture_Data_Pipeline_Graph_ANCHORED_VERIFIED.md",
  "1.0_VISION/Architecture_Anchors/receipts.json",
  "1.0_VISION/Architecture_Anchors/section_inventory.json",
  "1.0_VISION/Architecture_Anchors/anchor_claim_task_crosswalk.csv",
  "1.0_VISION/Architecture_Anchors/anchor_atomic_requirement_crosswalk.json",
  "1.0_VISION/Architecture_Anchors/anchor_atomic_requirement_crosswalk.csv",
  "1.0_VISION/Architecture_Anchors/anchor_conflict_ledger.csv",
  "1.0_VISION/Architecture_Anchors/source_contract_receipts.json",
  ...
  "consolidation/legacy_handoff_manifest.json",
  "consolidation/legacy_handoff_crosswalk.csv",
  "consolidation/retired_shadow_manifest.json",
  "consolidation/archive_receipt.json",
...
function verifyVisionNavigationLinks(navigationArtifacts) {
  ...
  const index = navigationArtifacts?.index ?? { records: {}, by_alias: {} };
  const allowedExternalReferences = new Set(navigationArtifacts?.source?.external_reference_paths ?? []);
  const failures = [];
  for (const relativeDoc of docs) {
    const docPath = path.join(pkgRoot, relativeDoc);
    const text = fs.readFileSync(docPath, "utf8");
    for (const finding of collectNavigationLinkFindings({ repoRoot, docPath, text, index, allowedExternalReferences })) {
      failures.push(`${finding.reason} ${finding.kind} in ${relativeDoc}: ${finding.target}`);
    }
  }
  assert(failures.length === 0, `Navigation links failed strict verification: ${failures.join("; ")}`);
}
