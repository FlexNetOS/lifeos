<script>
  // LifeOS — AIChat SFC (Svelte port of AIChat.vue)
  // Lightweight popup chat panel anchored to the AI avatar. Prototype-grade:
  // keeps a local message log + canned reply. Wire to real LifeOS via store later.
  import { onMount, onDestroy, tick } from "svelte";
  import { useLifeos } from "@/stores/lifeos.js";
  import { bindStore } from "@/lib/pinia-bridge.svelte.js";
  import Icon from "./Icon.svelte";

  const lifeos = useLifeos();
  const lifeosState = bindStore(lifeos, ["aiMessages", "workspace"]);

  let { anchor, onclose } = $props(); // anchor: { x, y }

  // Position the chat above-and-left of the avatar.
  const PANEL_W = 360, PANEL_H = 420;
  let panelStyle = $derived.by(() => {
    const x = Math.max(16, Math.min(window.innerWidth  - PANEL_W - 16, anchor.x - PANEL_W + 76));
    const y = Math.max(16, anchor.y - PANEL_H - 12);
    return `left: ${x}px; top: ${y}px; width: ${PANEL_W}px; height: ${PANEL_H}px;`;
  });

  let messages = $derived(lifeosState.aiMessages);
  let draft = $state("");
  let scroller = $state(null);

  // Multi-modal input state
  let micOn = $state(false);
  let camOn = $state(false);
  let moreOpen = $state(false);
  let moreEl = $state(null);
  let fileInput = $state(null);
  let attachments = $state([]);

  const ATTACH_META = {
    screen:  { icon: "monitor",  label: "Screen share" },
    image:   { icon: "image",    label: "Image" },
    audio:   { icon: "music",    label: "Audio clip" },
    link:    { icon: "link",     label: "Link" },
    agent:   { icon: "users-2",  label: "Agent hand-off" },
    command: { icon: "terminal", label: "Slash command" },
    file:    { icon: "file",     label: "File" },
  };
  const addAttach = (kind) => {
    attachments.push({ ...ATTACH_META[kind], kind });
    moreOpen = false;
  };
  const pickFile = () => fileInput?.click();
  const onFiles = (e) => {
    const files = e.target.files || [];
    for (const f of files) attachments.push({ icon: "file", label: f.name, kind: "file" });
    e.target.value = "";
  };

  // Close "more" menu on outside click / Escape
  const onDocMouse = (e) => { if (moreOpen && moreEl && !moreEl.contains(e.target)) moreOpen = false; };
  const onDocKey   = (e) => { if (e.key === "Escape") moreOpen = false; };
  onMount(() => { document.addEventListener("mousedown", onDocMouse); document.addEventListener("keydown", onDocKey); });
  onDestroy(() => { document.removeEventListener("mousedown", onDocMouse); document.removeEventListener("keydown", onDocKey); });

  const send = async () => {
    const text = draft.trim();
    if (!text && attachments.length === 0) return;
    const attachLabel = attachments.length
      ? ` · ${attachments.length} attachment${attachments.length === 1 ? "" : "s"}`
      : "";
    lifeos.sendAiMessage((text || "(no message)") + attachLabel, { source: "chat" });
    draft = "";
    attachments = [];
    await tick();
    scroller?.scrollTo({ top: 1e9, behavior: "smooth" });
  };

  const handleKey = (e) => {
    if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); }
  };

  onMount(async () => {
    await tick();
    scroller?.scrollTo({ top: 1e9 });
  });
</script>

<div class="ai-chat" style={panelStyle} role="dialog" aria-label="LifeOS AI chat">
  <header class="ai-chat-head">
    <span class="ai-chat-id">
      <span class="ai-chat-dot"></span>
      <strong>LifeOS</strong>
      <span class="ai-chat-meta">online · context: {lifeosState.workspace?.title || "overview"}</span>
    </span>
    <button class="ai-chat-close" onclick={() => onclose?.()} aria-label="Close chat">
      <Icon name="x" size={14} />
    </button>
  </header>
  <div class="ai-chat-log" bind:this={scroller}>
    {#each messages as m, i (i)}
      <div class="ai-chat-msg {m.role}">
        <span class="ai-chat-bubble">{m.text}</span>
      </div>
    {/each}
  </div>
  <form class="ai-chat-input" onsubmit={(e) => { e.preventDefault(); send(); }}>
    <div class="ai-chat-tools" role="toolbar" aria-label="Input modalities">
      <button type="button" class="ai-chat-tool" class:active={micOn} aria-pressed={micOn}
              onclick={() => micOn = !micOn} title="Voice · push-to-talk" aria-label="Toggle mic">
        <Icon name={micOn ? "mic" : "mic-off"} size={14} />
      </button>
      <button type="button" class="ai-chat-tool" class:active={camOn} aria-pressed={camOn}
              onclick={() => camOn = !camOn} title="Video · share what you see" aria-label="Toggle camera">
        <Icon name={camOn ? "video" : "video-off"} size={14} />
      </button>
      <button type="button" class="ai-chat-tool" onclick={pickFile} title="Upload file or folder" aria-label="Upload">
        <Icon name="folder-up" size={14} />
      </button>
      <input type="file" multiple bind:this={fileInput} hidden onchange={onFiles} />
      <div class="ai-chat-tool-more" bind:this={moreEl}>
        <button type="button" class="ai-chat-tool" class:active={moreOpen} aria-expanded={moreOpen}
                onclick={() => moreOpen = !moreOpen} title="More inputs" aria-label="More inputs">
          <Icon name="plus" size={14} />
        </button>
        {#if moreOpen}
          <div class="ai-chat-more-menu" role="menu">
            <button type="button" onclick={() => addAttach("screen")} role="menuitem"><Icon name="monitor" size={13} /> Share screen</button>
            <button type="button" onclick={() => addAttach("image")} role="menuitem"><Icon name="image" size={13} /> Image</button>
            <button type="button" onclick={() => addAttach("audio")} role="menuitem"><Icon name="music" size={13} /> Audio clip</button>
            <button type="button" onclick={() => addAttach("link")} role="menuitem"><Icon name="link" size={13} /> Paste link</button>
            <button type="button" onclick={() => addAttach("agent")} role="menuitem"><Icon name="users-2" size={13} /> Hand-off to agent</button>
            <button type="button" onclick={() => addAttach("command")} role="menuitem"><Icon name="terminal" size={13} /> Slash command</button>
          </div>
        {/if}
      </div>
    </div>
    <div class="ai-chat-row">
      <input bind:value={draft}
             onkeydown={handleKey}
             placeholder="Ask LifeOS anything · ↵ to send"
             aria-label="Message LifeOS"
             autocomplete="off" />
      <button type="submit" class="ai-chat-send" aria-label="Send" disabled={!draft.trim() && attachments.length === 0}>
        <Icon name="send" size={14} />
      </button>
    </div>
    {#if attachments.length}
      <ul class="ai-chat-attachments" aria-label="Pending attachments">
        {#each attachments as a, i (i)}
          <li>
            <Icon name={a.icon} size={11} />
            <span>{a.label}</span>
            <button type="button" class="ai-chat-attach-x" onclick={() => attachments.splice(i, 1)} aria-label="Remove attachment"><Icon name="x" size={10} /></button>
          </li>
        {/each}
      </ul>
    {/if}
  </form>
</div>
