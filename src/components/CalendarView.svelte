<script>
  ...
  let workspaceId = $derived(sub?.workspaceId || "");
  const WORK_TAGS    = new Set(["Work"]);
  const PERSONAL_TAGS = new Set(["Personal", "Family"]);

  let tagFilter = $derived.by(() => {
    if (workspaceId === "work")     return WORK_TAGS;
    if (workspaceId === "personal") return PERSONAL_TAGS;
    return null; // null = show all
  });

  // Pull agenda from global data.
  let agenda = $derived.by(() => {
    const raw = globalThis.LIFEOS_DATA?.dashboardCanvas?.agenda || [];
    const f = tagFilter;
        ...
        >
          ...
          <ul class="cal-event-list">
            {#each events as ev, i (i)}
              ...
              >
                <span class="cal-event-accent" aria-hidden="true"></span>
