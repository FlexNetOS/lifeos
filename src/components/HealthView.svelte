<script>
  // LifeOS — HealthView SFC (Svelte port of HealthView.vue)
  ...
  let heartData = $derived(health.heart);
  let bpms = $derived(heartData.map((s) => s.bpm));
  let bpmMin = $derived(Math.min(...bpms));
  let bpmMax = $derived(Math.max(...bpms));
  let bpmCurrent = $derived(bpms[bpms.length - 1] ?? 0);
  let bpmMinIdx = $derived(bpms.indexOf(bpmMin));
  let bpmMaxIdx = $derived(bpms.indexOf(bpmMax));
  ...
  <div class="health-body">
    ...
    <div class="health-main">
      ...
      <div class="health-stats-grid">
        {#each health.metrics as m (m.id)}
          <div
            class="health-stat-card"
            role="img"
            aria-label={`${m.label}: ${m.value} ${m.unit}, ${m.delta}`}
            style="background: linear-gradient(180deg, {toneOf(m.tone).bg} 0%, transparent 60%), var(--bg-2); border-color: {toneOf(m.tone).bd};"
          >
      ...
      <section class="health-chart-card" role="region" aria-labelledby="sleep-title">
        ...
        >
          ...
          <line
            ...
            aria-hidden="true"
          />
          {#each health.sleep as night, i (night.day)}
            <g aria-hidden="true">
              <rect
                x={i * 32 + 4}
      ...
      <section class="health-chart-card" role="region" aria-labelledby="activity-title">
        ...
        <div class="health-activity-row">
          {#each health.activity as day, i (day.day)}
            <div
              class="health-activity-day"
              class:health-activity-today={i === health.activity.length - 1}
            >
              <svg
                width={(R_OUTER + 2) * 2}
                height={(R_OUTER + 2) * 2}
