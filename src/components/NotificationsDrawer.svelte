<script>
  // LifeOS — NotificationsDrawer SFC (Svelte port of NotificationsDrawer.vue)
  ...
  // (bindStore mirrors store arrays as raw shallow copies — see the hardening
  // notes in pinia-bridge.svelte.js — so .includes on the snapshot is plain
  // Array.prototype.includes with no Vue instrumentation in the way.)
  let visible = $derived.by(() => {
    const all = globalThis.LIFEOS_DATA?.notifications || [];
    const dismissed = lifeosState.dismissedNotificationIds;
    return all.filter((n) => !dismissed.includes(n.id));
  });

  let unreadCount = $derived.by(() =>
    visible.filter(
      (n) => n.unread && !lifeosState.readNotificationIds.includes(n.id),
    ).length,
  );

  function isRead(n) {
    return !n.unread || lifeosState.readNotificationIds.includes(n.id);
  }
  ...
  // Vue: watch(() => lifeos.notificationsDrawerOpen, (open) => { if (open) nextTick(...) })
  $effect(() => {
    if (lifeosState.notificationsDrawerOpen) {
      tick().then(() => {
        closeEl?.focus();
      });
    }
    ...
    >
      ...
      <header class="notif-header">
        <div class="notif-header-left">
          ...
          {#if unreadCount > 0}
            <span class="notif-unread-chip" aria-label="{unreadCount} unread">
              {unreadCount}
            </span>
          {/if}
        </div>
      ...
      <div class="notif-body">
        ...
        {:else}
          <!-- Notification list -->
          <!-- svelte-ignore a11y_no_redundant_roles -- role="list" is kept byte-identical to the Vue original; the specs query [role='listitem'] literally -->
          <ul role="list" aria-label="Notifications" class="notif-list">
            {#each visible as n (n.id)}
              ...
              >
                <span
                  class="notif-icon-wrap tone-bg-{n.tone}"
                  aria-hidden="true"
                >
...
/* Vue's scoped version of this rule reached the child Icon's root svg (child
   component roots inherit the parent scope attr in Vue); Svelte scoping can't
   see into child components, so :global keeps the rule applying identically. */
.notif-empty :global(.lucide),
