<script>
  ...
  let pane = $derived(item.pane || "canvas");
  ...
  let fileItems = $derived.by(() => {
    const ai = globalThis.LIFEOS_DATA?.workspaces?.ai?.sections || [];
  ...
  let expandedDirs = $state(new Set(["src", "src/components", "src/stores", "src/lib", "src-tauri", "src-tauri/src", "tests"]));
  ...
  let flatTree = $derived.by(() => {
    ...
    const walk = (node, level) => {
      if (node.type === "dir") {
        ...
        if (level === 0 || expandedDirs.has(node.path)) {
  ...
  let langLabel = $derived({
    ...
    html: "HTML",
    css: "CSS",
  }[fileExt] || (fileExt ? fileExt.toUpperCase() : "File"));
  ...
  let previewHtml = $derived.by(() => {
    ...
    if (ext === "json") {
      try { return "<pre>" + JSON.stringify(JSON.parse(fileContent), null, 2) + "</pre>"; }
  ...
  // ($effect runs immediately after mount and on every activePath change,
  // mirroring Vue's watch(activePath, ..., { immediate: true }); untrack keeps
  // the openTabs/fileItems reads from re-triggering the effect.)
  let openTabs = $state([]);
  $effect(() => {
    const p = activePath;
    if (!p) return;
    untrack(() => {
      if (!openTabs.find(t => t.path === p)) {
        const f = fileItems.find(x => x.path === p);
        openTabs.push({ path: p, icon: f?.icon || "file" });
      }
    });
  });
  ...
  let filesView = $state("split");   // "editor" | "preview" | "split"

  // File-pick handler: routes via the store so router + breadcrumb stay in sync
  const openFile = (path) => {
    const fileEntry = fileItems.find(f => f.path === path);
    if (!fileEntry) return;
    activeFolder = null;
...
<div class="canvas op-canvas">
  <header class="op-head">
    <button class="sub-back" onclick={() => lifeos.clearSub()} aria-label="Back to dashboard">
    ...
    <aside class="op-left">
      <div class="op-tools" role="toolbar" aria-label="Drawing tools">
        {#each tools as t (t.id)}
          <button class="op-tool" class:active={activeTool === t.id}
      ...
      <div class="op-layers" aria-label="Layers">
        ...
        {#each layers as layer (layer.id)}
          <button class="op-layer" class:selected={layer.selected}
                  style="padding-left: {10 + layer.depth * 12}px;"
            ...
            {#if layer.type === "instance"}
              <Icon name="link" size={9} class="op-layer-instance" />
            {/if}
          </button>
        {/each}
      </div>
    ...
    <main class="op-canvas-wrap">
      <div class="op-canvas-head">
        ...
        <div class="op-canvas-actions">
          <button class="op-mini-btn" title="Zoom out" aria-label="Zoom out"><Icon name="zoom-out" size={12} /></button>
          <span class="op-zoom">100%</span>
          <button class="op-mini-btn" title="Zoom in" aria-label="Zoom in"><Icon name="zoom-in" size={12} /></button>
          <button class="op-mini-btn" title="Toggle grid" aria-label="Toggle grid"><Icon name="grid-3x3" size={12} /></button>
        </div>
      </div>
      <div class="op-canvas-stage" data-pane={pane}>
        ...
        <!-- COMPONENTS PANE -->
        {:else if pane === "components"}
          <div class="op-grid">
            {#each ["MenuRow", "Badge", "Icon", "AIAvatar", "AIChat", "SectionHeader", "StatCard", "TeamCard"] as c (c)}
              <div class="op-comp-card">
        ...
        <!-- AI CHAT PANE -->
        {:else if pane === "ai-chat"}
          <div class="op-ai-chat">
            <div class="op-ai-head">
              <span class="op-ai-head-meta">
                <span class="op-dot online"></span>
                Shared with the floating AI panel · {lifeosState.aiMessages.length} messages
              </span>
              <button type="button" class="op-mini-btn" title="Clear conversation" aria-label="Clear conversation" onclick={() => lifeos.clearAiMessages()}>
                <Icon name="rotate-ccw" size={11} />
              </button>
            </div>
            <div class="op-ai-log">
              {#each lifeosState.aiMessages as m, i (i)}
                <div class="op-ai-msg {m.role}">
                  <span>{m.text}</span>
                </div>
              {/each}
            </div>
            <form class="op-ai-input" onsubmit={(e) => { e.preventDefault(); sendAi(); }}>
              <input bind:value={aiDraft}
...
#   • tokens · linter · export · figma-compat · screenshot · query (xpath)</pre>
          <div class="op-status-row">
            <span class="op-dot online"></span> stdio listening
            <span class="op-dot online"></span> http://localhost:3100
            <span class="op-dot warn"></span> require API key for chat
          </div>
        ...
        {:else if pane === "tokens"}
          <div class="op-tokens">
            {#each [
              ...
              <div class="op-token">
                <span class="op-token-swatch" style="background: {t.hex};"></span>
        ...
        {:else if pane === "export"}
          <div class="op-grid">
            {#each ["PNG @1x", "PNG @2x", "PNG @3x", "SVG", "JPG q90", "WEBP", ".fig (file)", "JSX · Tailwind"] as e (e)}
              <button class="op-comp-card">
            ...
            <aside class="op-files-tree" aria-label="File tree">
              <div class="op-pane-head explorer">
                ...
                <span class="op-pane-head-actions">
                  <button class="op-mini-btn" title="New file" aria-label="New file" onclick={newFile}>
                    <Icon name="file-plus" size={11} />
                  </button>
                  <button class="op-mini-btn" title="Refresh" aria-label="Refresh">
              ...
              <div class="op-files-tree-body">
                {#each flatTree as row ((row.kind === "dir" ? "d:" : "f:") + row.path)}
                  {#if row.kind === "dir"}
                    <div class="op-tree-row-wrap">
                      <button class="op-tree-row dir"
                              class:active={activeFolder === row.path}
                              style="padding-left: {8 + (row.level - 1) * 12}px;"
                              title={row.path}
                      ...
                      <button class="op-tree-chev"
                              ...
                              onclick={(e) => { e.stopPropagation(); toggleDir(row.path); }}>
                        <Icon name={expandedDirs.has(row.path) ? "chevron-down" : "chevron-right"} size={10} />
                      </button>
                    </div>
                  {:else}
                    <button class="op-tree-row file"
                            class:active={activePath === row.path}
                            style="padding-left: {24 + (row.level - 1) * 12}px;"
                            title={row.path}
            ...
            <section class="op-files-main">
              ...
              <div class="op-files-tabs" role="tablist" aria-label="Open files">
                {#each openTabs as t (t.path)}
                  <button class="op-files-tab" class:active={activePath === t.path}
                          role="tab"
                ...
                {/each}
                {#if !openTabs.length}
                  <span class="op-files-tab-empty">no files open</span>
                {/if}
              </div>

              <!-- Folder dashboard: shown when activeFolder is set and no file is active -->
              {#if folderDashboard && !activePath}
                <div class="op-folder-dash">
                  <header class="op-folder-head">
                    ...
                    <button class="op-mini-btn" title="New file in this folder" aria-label="New file in this folder"
                            onclick={newFile}>
                  ...
                  <div class="op-folder-stats">
                    ...
                    {#each folderDashboard.byExt.slice(0, 4) as e (e.ext)}
                      <div class="op-folder-stat">
                  ...
                  {#if folderDashboard.subdirs.length}
                    <section class="op-folder-section">
                      <h4>Subfolders</h4>
                      <div class="op-folder-grid">
                        {#each folderDashboard.subdirs as s (s)}
                          <button class="op-folder-card folder"
                                  onclick={() => openFolder(folderDashboard.path + "/" + s)}>
                  ...
                  <section class="op-folder-section">
                    <h4>Files</h4>
                    <div class="op-folder-grid">
                      {#each folderDashboard.children as f (f.path)}
                        <button class="op-folder-card"
                                onclick={() => openFile(f.path)}>
                          <Icon name={f.icon || "file"} size={16} />
                          <div class="op-folder-card-body">
                            <span class="op-folder-card-name">{f.path.split("/").pop()}</span>
                            {#if f.meta}<span class="op-folder-card-meta">{f.meta}</span>{/if}
                          </div>
                          <Icon name="chevron-right" size={11} />
                        </button>
                      {/each}
              ...
              {:else if activePath}
                <header class="op-files-head">
                  <Icon name="file-code" size={13} />
                  <span class="op-files-path">
                    {#each activePath.split("/") as seg, i (i)}
                      {#if i > 0}<span class="op-files-path-sep">/</span>{/if}
                      <span class={i === activePath.split("/").length - 1 ? "op-files-path-leaf" : ""}>{seg}</span>
                    {/each}
                  </span>
                  ...
                  <div class="op-files-view-switch" role="tablist" aria-label="View">
                    ...
                    <button role="tab" aria-selected={filesView === "split"} class:active={filesView === "split"} onclick={() => filesView = "split"} title="Editor + Preview">
                      <Icon name="columns-2" size={11} /> Split
                    </button>
                    <button role="tab" aria-selected={filesView === "preview"} class:active={filesView === "preview"} onclick={() => filesView = "preview"} title="Preview only">
                      <Icon name="eye" size={11} /> Preview
                    </button>
                  </div>
                  <button class="op-mini-btn" title="Open in OpenPencil canvas" aria-label="Open in OpenPencil canvas"
                          onclick={() => nav.pickSub({ icon: "pen-tool", label: "OpenPencil canvas", view: "open-pencil", pane: "canvas" }, "UI-Editor")}>
              ...
              {#if activePath}
                <div class="op-files-body" data-view={filesView}>
                  {#if filesView !== "preview"}
                    <div class="op-files-editor" aria-label="File editor">
                  ...
                  {/if}
                  {#if filesView !== "editor"}
                    <div class="op-files-preview" aria-label="File preview">
                      <div class="op-files-preview-head">
                        <Icon name="eye" size={11} />
                        <span>Preview</span>
                        <span class="op-files-preview-ext">.{fileExt}</span>
                      </div>
                      <div class="op-files-preview-body">{@html previewHtml}</div>
                    </div>
                  {/if}
                </div>
              {:else if !folderDashboard}
                <div class="op-files-empty">
    ...
    <aside class="op-right">
      <div class="op-pane-head">
        <span>Inspector</span>
        <button class="op-mini-btn" title="Properties" aria-label="Properties"><Icon name="sliders-horizontal" size={12} /></button>
      </div>
      <div class="op-insp">
        <div class="op-insp-section">
          <div class="op-insp-label">Selection</div>
          <input class="op-insp-name" bind:value={inspector.name} aria-label="Selected layer name" />
        </div>
        <div class="op-insp-section">
        ...
        <div class="op-insp-section">
          ...
          <div class="op-insp-color"><span class="op-swatch" style="background: {inspector.fill};"></span><input value={inspector.fill} aria-label="Fill color" /></div>
          <div class="op-insp-label">Stroke</div>
          <div class="op-insp-color"><span class="op-swatch" style="background: {inspector.stroke};"></span><input value={inspector.stroke} aria-label="Stroke color" /></div>
        </div>
      </div>
    </aside>
  </section>
