<script>
  // LifeOS — TelemetryWidget SFC (Svelte port of TelemetryWidget.vue)
  ...
  const lifeosState = bindStore(lifeos, ["telemetryEnabled", "telemetryRefreshMs"]);
  ...
  let telemetryEnabled = $derived(lifeosState.telemetryEnabled !== false);
  let refreshMs = $derived.by(() => {
    const n = Number(lifeosState.telemetryRefreshMs);
...
<section class="telemetry-row" aria-label="System telemetry">
  {#if !isTauri}
    <div class="telemetry-placeholder" role="note">
  ...
  {:else}
    ...
    <div class="telemetry-card"
      ...
      <div class="telemetry-value">
        {#if snapshot}{snapshot.cpu_percent.toFixed(0)}<span class="telemetry-unit">%</span>{:else}—{/if}
      </div>
      <div class="telemetry-bar" aria-hidden="true">
        <span class="telemetry-bar-fill telemetry-bar-fill--cyan"
              style="width: {(snapshot ? Math.max(0, Math.min(100, snapshot.cpu_percent)) : 0)}%"></span>
      </div>
    </div>
    ...
    <div class="telemetry-card"
      ...
      <div class="telemetry-value">
        {#if snapshot}{formatBytes(snapshot.memory_used_bytes)}{:else}—{/if}
      </div>
      <div class="telemetry-meta">
        {#if snapshot}of {formatBytes(snapshot.memory_total_bytes)}{:else}&nbsp;{/if}
      </div>
      <div class="telemetry-bar" aria-hidden="true">
        <span class="telemetry-bar-fill telemetry-bar-fill--purple"
              style="width: {memoryPercent(snapshot)}%"></span>
      </div>
    </div>
    ...
    <div class="telemetry-card"
      ...
      <div class="telemetry-value telemetry-value--small">
        {#if snapshot}{formatBytes(snapshot.network_rx_bytes)}{:else}—{/if}
      </div>
      <div class="telemetry-meta">
        {#if snapshot}rx · tx {formatBytes(snapshot.network_tx_bytes)}{:else}&nbsp;{/if}
      </div>
      <div class="telemetry-bar" aria-hidden="true">
        <span class="telemetry-bar-fill telemetry-bar-fill--green" style="width: 100%"></span>
      </div>
    </div>
    ...
    <div class="telemetry-card"
      ...
      <div class="telemetry-value telemetry-value--small">
        {#if snapshot}{formatUptime(snapshot.uptime_seconds)}{:else}—{/if}
      </div>
      <div class="telemetry-meta">
        {#if snapshot}{snapshot.hostname}{:else}&nbsp;{/if}
      </div>
      <div class="telemetry-bar" aria-hidden="true">
        <span class="telemetry-bar-fill telemetry-bar-fill--cyan" style="width: 100%"></span>
      </div>
    </div>
  {/if}
</section>
