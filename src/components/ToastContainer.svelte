<script>
  // LifeOS — ToastContainer SFC (Svelte port of ToastContainer.vue)
  ...
  const toastsState = bindStore(toasts, ["items"]);
...
<div use:portal class="toast-stack" role="region" aria-label="Notifications" aria-live="polite">
  <!-- Vue's <TransitionGroup name="toast" tag="div" class="toast-inner" move-class="toast-move">
       renders a plain <div class="toast-inner"> wrapper at rest; the toast-enter/leave/move
       classes only exist transiently during animations. The final-state DOM is replicated
       here without an animation library; the transition class hooks stay in <style> below. -->
  <div class="toast-inner">
    {#each toastsState.items as toast (toast.id)}
      <div
        class="toast-item toast-{toast.variant}"
        role={VARIANT_ROLE[toast.variant]}
        onmouseenter={() => toasts.pauseTimer(toast.id)}
        onmouseleave={() => toasts.resumeTimer(toast.id)}
      >
