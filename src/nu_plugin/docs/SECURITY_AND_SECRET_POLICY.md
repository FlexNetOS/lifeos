...
Unsafe-execution approval does not override this policy. When this policy and
the unsafe-capture policy both apply, the more restrictive requirement wins.
...
| `local-only` | Stored only in an explicitly selected, untracked, operator-owned local redb | Export hashes and metadata only. This mode is never implied by a store path and is never exposed through MCP. |
...
|---|---|
| Match in `refuse` mode | Abort the affected blob or artifact and emit a sanitized `validation_errors` row. |
| Match in `hash-only` mode | Retain metadata and hash only; record that raw persistence was denied. |
...
- raw source or blob reads and full-file dumps;
- unbounded table dumps or pagination bypass;
- unsafe build or proc-macro capture;
