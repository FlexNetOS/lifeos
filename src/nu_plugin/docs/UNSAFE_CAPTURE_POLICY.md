...
Static detection of `build.rs`, proc-macro crates and invocations, include
edges, and native/linker declarations is allowed during a normal read-only
scan. Executing build scripts or procedural macros is not read-only: it may
run arbitrary code, read files or environment values, invoke tools, access the
...
Unsafe approval does not authorize disclosure or retention forbidden by
...
- canonical source repository or workspace path and before-state Git status
  plus a file-hash manifest;
- exact command, CodeDB version, toolchain, target, profile, features, and
  relevant lock/config checksums;
- permitted build-script and proc-macro scope;
...
source-tree write, scope escape, undeclared process or tool invocation, or
...
- native libraries, link paths and arguments, linker, pkg-config, and `cc`
  invocations;
- OUT_DIR/generated-file identities, paths, sizes, and content hashes;
- toolchain/context metadata, exit status, timestamps and durations, and
  failure logs;
...
declared artifact directory using the bound approval and evidence record. They
...
- missing, expired, or mismatched approval provenance refuses before
  execution;
