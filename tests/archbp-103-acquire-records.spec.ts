...
describe("ARCHBP-103 acquire records prior state", () => {
  test("acquire captures the pre-acquire host state before taking control", async () => {
    ...
    try {
      ...
      const plane = new HostControlPlane({ auditPath, registry: defaultRegistry(dir, { port: 23483 }) });
      const { prior } = await plane.acquire("loopback-port-23483");
      expect(prior).toEqual({ port: 23483, free: true }); // real pre-state
      ...
      await plane.release("loopback-port-23483");
    } finally {
  ...
  test("a failing acquire auto-releases and is audited (nothing stays half-held)", async () => {
    ...
    try {
      const auditPath = join(dir, "a.jsonl");
      const registry = defaultRegistry(dir, { port: 23483 });
