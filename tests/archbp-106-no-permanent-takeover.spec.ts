...
describe("ARCHBP-106 no permanent takeover", () => {
  test("controls are time-bounded: an expired lease auto-releases on sweep", async () => {
    ...
    try {
      ...
      const plane = new HostControlPlane({ auditPath, registry: defaultRegistry(dir, { port: 23486 }) });
  ...
  test("auto-release on failure: a failed acquire leaves zero holds", async () => {
    ...
    try {
      const registry = defaultRegistry(dir, { port: 23486 });
  ...
  test("the little-brother invariant is tested: host functions DURING a hold", async () => {
    ...
    try {
      const plane = new HostControlPlane({ auditPath: join(dir, "a.jsonl"), registry: defaultRegistry(dir, { port: 23486 }) });
      await plane.acquire("loopback-port-23486");
      ...
      await new Promise<void>((res, rej) => { other.once("error", rej); other.listen(23487, "127.0.0.1", () => res()); });
      await new Promise((res) => other.close(res));
      await plane.release("loopback-port-23486");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
