...
describe("ARCHBP-108 commandeer + clean release end-to-end", () => {
  test("commandeer demonstrated, clean release verified, OS normal throughout", async () => {
    ...
    try {
      ...
      expect(await portFree(38471)).toBe(true);
      await plane.acquire("loopback-port-38471", { ttlMs: 30000 });
      expect(await portFree(38471)).toBe(false); // LifeOS holds it for real
      ...
      const bindError = await new Promise<string>((res) => {
        ...
        denied.listen(38471, "127.0.0.1", () => res("BOUND"));
      });
      ...
      await new Promise<void>((res, rej) => { other.once("error", rej); other.listen(38473, "127.0.0.1", () => res()); });
